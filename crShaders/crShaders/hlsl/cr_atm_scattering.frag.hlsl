    ...
    for (int i = 0; i < sqrtSamples; i++) {
        for (int j = 0; j < sqrtSamples; j++) {
            ...
            for (float stepI = 0.0; stepI < mulScattSteps; stepI += 1.0) {
                ...
                float3 sunTransmittance = getValFromTLUT(MESH_TEXTURES[1], SAMPLER, tLUTRes, newPos, sunDir);
            ...
            if (groundDist > 0.0) {
                ...
                if (dot(pos, sunDir) > 0.0) {
                    ...
                    lum += transmittance * groundAlbedo * getValFromTLUT(MESH_TEXTURES[1], SAMPLER, tLUTRes, hitPos, sunDir);
                }
            }
...
float4 main(PixelShaderInput input) : SV_TARGET
{
    ...
    */
    ...
    float3 lum, f_ms;
    getMulScattValues(pos, sunDirAlt, lum, f_ms);
